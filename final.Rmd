...
Environmental sustainability is the practice of using renewable and environmentally friendly practices in order to protect the earth's resouces and provide for future generations. In this report we seek to understand the environmental impact of every state by analyzing data on landfills, water withdrawals, and power plant emissions for each state. As organic materials in landfills decompose they emit ozonedepleting subatnaces, also known as landfill gas (LFG), which have a negative effect on the environemnt and contribute to global warming. However, some landfills have LFG systems in place that collect emissions and lessen the amount of greenhouse gases emited by the lanfill. Water withdrawals refer to the amount of water withdrawn from a natural groundwater or surface water source. There are many activities such as agriculture, power geneartion, and manufacturing that require water contribute to the level of water withdrawals across the country. Water withdrawal has many associated problems including drought, however, more efficient transfer of water, stormwater capture and water recycling can help reduce the impact of water withdrawals. Power plants use a variety of fuels to produce energy, including fossil fuels such as coal, oil, and natural gas. When these fuels are burned, they produce greenhouse gases, which contribute to global warming. Other newable sources of energy such as solar, wind, hydro, and geothermal do not emit greenhouse gases and are therefore much better for the environment. Different states may have different laws and regualtions regarding these three areas and therefore have varying levels of environmental impact. Using data on landfills, water withdrawals and power plant emissions, we have created scores for ranking the environmental impact of each state by combining and weighing serveral sustainability factors based on their importance and impact.
...
As for the rankings, the overall order did not surprise us.Washington has the best overall score and the state of Wyoming has the worst score. Here is a breakdown for Washington’s ranks on our 6 factors.
...
Wyoming ranked second to last in the total emissions and didn’t do well in the renewables or combustible power sections as well. This is probably in part due to the fact that they are the number one resource of coal in the United States, mining almost 400 million tons of coal per year (Wikipedia Coal). Using coal to produce energy emits large amounts of Sulfur Dioxide (SO2) and Nitrogen Oxides( NOx) (GreenPeace).


Another variable that struck us as interesting was how low California’s score was. California is regarded one of the most strict in terms of environmental regulations. Looking at California's individual scores we see that they are 3rd to last for waste.

```{r eval=TRUE, echo=FALSE, warning=FALSE, evaluted=FALSE, error=FALSE, message=FALSE}
kable(ca.ranks)
```

We thought this was odd so we looked further into the reasoning behind why their score is so low. The waste score is calculated by the sum of all of the waste divided by the population. Going back and looking at the data we noticed that some of the states don’t have a total for their waste, meaning this data was never collected. Here is chart showing the percentage of landfills that have their waste data collected. (Note the states with NA have all of their landfill data collected.)

```{r eval=TRUE, echo=FALSE, warning=FALSE, evaluted=FALSE, error=FALSE, message=FALSE}
kable(head(landfill.withna))
kable(tail(landfill.withna, 15L))
```

As you can see California has 94% of their landfill data collected, while Idaho only has 25% of their landfill data collected. This means that Idaho could have 75% more trash than we accounted for. This error skewes all of our data significantly and explains why California, has a bad score for waste.

If we multiplied their total waste by the percentage of waste that each state has, here is the new scores if we had adjusted for missing variables.
...
As you can see, the overall scores and map hasn't changed too much but California's overall score went up by about 10 points. This error changes the HELP ME OMG 
