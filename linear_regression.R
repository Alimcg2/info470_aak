...
a <- plotFunc(df$emissions,'rgba(10, 36, 99, .6)', 'rgba(10, 36, 99, .9)', '', "Emissions Score")
b <- plotFunc(df$withdrawals,'rgba(63, 136, 197, .6)', 'rgba(63, 136, 197, .9)', "", "Withdrawals Score")
c <- plotFunc(df$waste.pop, 'rgba(232, 151, 44, .6)', 'rgba(232, 151, 44, .9)',"", "Waste Score")
d <- plotFunc(df$lfg.collected, 'rgba(239, 219, 127, .6)', 'rgba(239, 219, 127, .9)', "", "LFG Score")
e <- plotFunc(df$renews, 'rgba(215, 38, 56, .6)', 'rgba(215, 38, 56, .9)', "", "Renewables Score")
f <- plotFunc(df$combustion, 'rgba(148, 16, 32, .6)', 'rgba(148, 16, 32, .9)', "", "Combustion Score")
...
p.emissions <- plot_ly(x = residuals(fit), y = df$emissions, mode = "markers", text = ~state.name, 
  ...
  layout(title = "",
         xaxis = list(zeroline = FALSE, title = "Residuals"),
...
h <- plot_ly(x = c(1:50), y = 1, type = "bar", text = df$state.name,  marker = list(color = 'rgb(198, 198, 198)')) %>% 
  layout(title = "",  font = list(family = "times"), 
